...
const Index = () => {
  ...
  const handelDetail = async (data) => {
    console.log("Tab:- ", data._id)
    await axios.get(`http://localhost:9000/company/reviews/${data._id}`).then(res => { setExpandedContent(res.data.compDetails); console.log(res.data); setAddReviewBtn(true) }).catch(err => console.log(err))
          ...
          companyList.company_data.length > 0 && companyList.company_data.map(data => (
            <div className="">
              ...
              <Accordion expanded={expanded === data._id} onChange={handleChange(data._id)}>
                <a className='accordion' onClick={() => handelDetail(data)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <Box sx={{display: "flex"}}>
                    ...
                    <AccordionSummary aria-controls="panel1d-content" id="panel1d-header">
                      <Box>
                        <Box sx={{ display: "flex", flexDirection: "column" }}>
                          <Typography sx={{ mt: 2, ml: 5, fontSize: 13, fontWeight: "medium", color: "gray" }}>Founded {data.founded.split("-")[0]} </Typography>
                ...
                <Box>
                  <div className=""><ArrowForwardIosIcon/></div>
                  {/* <button>Add Review</button> */}
                </Box>
                </a>
                <AccordionDetails>
                  {
                    expandedContent !== undefined && expandedContent.comment.length > 0 ? expandedContent.comment.map(item => (
                      <>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                          <Box sx={{ display: 'flex' }}>
                            <Tooltip title="Open settings">
                              <IconButton sx={{ p: 0 }}>
                                <Avatar alt="Remy Sharp" src="https://th.bing.com/th/id/OIP.26kdh2R6mmJ3owBJxolIhAHaHa?w=157&h=180&c=7&r=0&o=5&dpr=1.3&pid=1.7" />
                              </IconButton>
                            </Tooltip>
                            {/* <Box>
                              <Typography sx={{ ml: 2, fontWeight: "bold"}}>{item.user_id.name}</Typography>
                              <Typography sx={{ ml: 2, fontWeight: "bold", color: "gray" }}>{item.createdAt}</Typography>
                            </Box> */}
                          </Box>
                        ...
                        <Typography sx={{ px: 7, mb: 8, mt: 1 }}> {item.review} </Typography>
                      </>
                    )) : <Box sx={{ display: 'flex', justifyContent: "center" }}>No Review found!!!</Box>
                  }
                  {expandedContent !== undefined && expandedContent.comment.length > 0 && <a className="flex text-center">See All</a>}
